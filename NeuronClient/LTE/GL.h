...
/* Check for errors. */
inline GL_Error::Enum GL_GetError() { return static_cast<GL_Error::Enum>(glGetError()); }
...
{
  glAttachShader(static_cast<unsigned int>(program), static_cast<unsigned int>(shader));
  DEBUG_GL_ERRORS;
}
...
inline void GL_BindAttribLocation(GL_Program program, unsigned int index, const char* name)
{
  glBindAttribLocation(static_cast<unsigned int>(program), index, name);
  DEBUG_GL_ERRORS;
}
...
{
  glBindBuffer(target, static_cast<unsigned int>(buffer));
  DEBUG_GL_ERRORS;
}

inline void GL_BindFragDataLocation(GL_Program program, unsigned int index, const char* name)
{
  glBindFragDataLocation(static_cast<unsigned int>(program), index, name);
  DEBUG_GL_ERRORS;
}

/* Bind a named framebuffer object. */
inline void GL_BindFramebuffer(GL_FramebufferTarget::Enum target, GL_Framebuffer buffer)
{
  glBindFramebuffer(target, static_cast<unsigned int>(buffer));
  DEBUG_GL_ERRORS;
}

/* Bind a named renderbuffer object. */
inline void GL_BindRenderbuffer(GL_RenderbufferTarget::Enum target, GL_Renderbuffer renderBuffer)
{
  glBindRenderbuffer(target, static_cast<unsigned int>(renderBuffer));
  DEBUG_GL_ERRORS;
}

/* Bind a named texture to a texturing target. */
inline void GL_BindTexture(GL_TextureTargetBindable::Enum target, GL_Texture texture)
{
...
/* Specify the blending function to be used for the source and destination
   surfaces. */
inline void GL_BlendFunc(GL_BlendFunction::Enum src, GL_BlendFunction::Enum dst)
{
  glBlendFunc(src, dst);
  DEBUG_GL_ERRORS;
}
...
inline void GL_BlendFuncSeparate(GL_BlendFunction::Enum srcRGB, GL_BlendFunction::Enum dstRGB, GL_BlendFunction::Enum srcAlpha,
...
inline void GL_BufferData(GL_BufferTarget::Enum target, int bufferSize, const void* initialData, GL_BufferUsage::Enum usage)
{
...
{
#ifdef BUILD_STRICT
  GL_FramebufferStatus::Enum status = (GL_FramebufferStatus::Enum)glCheckFramebufferStatus(GL_FramebufferTarget::Draw); DEBUG_GL_ERRORS; if
...
inline void GL_Color(float r, float g, float b) { glColor3f(r, g, b); }

/* Set the current color. */
inline void GL_Color(float r, float g, float b, float a) { glColor4f(r, g, b, a); }

/* Compile a shader object. */
inline void GL_CompileShader(GL_Shader shader)
{
  glCompileShader(static_cast<unsigned int>(shader));
  DEBUG_GL_ERRORS;
}

inline void GL_CopyTexImage2D(GL_TextureTarget::Enum target, int level, GL_TextureFormat::Enum internalFormat, int x, int y, int width,
...
{
  auto p = GL_Program(glCreateProgram());
  DEBUG_GL_ERRORS;
  return p;
}
...
{
  auto s = GL_Shader(glCreateShader(type));
  DEBUG_GL_ERRORS;
  return s;
}
...
{
  glDeleteProgram(static_cast<unsigned int>(program));
  DEBUG_GL_ERRORS;
}
...
{
  glDeleteShader(static_cast<unsigned int>(shader));
  DEBUG_GL_ERRORS;
}
...
inline void GL_DrawElements(GL_DrawMode::Enum mode, int indexCount, GL_IndexFormat::Enum indexFormat, const void* indexData)
{
  GL_CheckFramebuffer();
...
inline void GL_FramebufferTexture2D(GL_FramebufferTarget::Enum framebufferTarget, GL_FramebufferAttachment::Enum attachment,
                                    GL_TextureTarget::Enum textureTarget, GL_Texture texture, int level)
{
  glFramebufferTexture2D(framebufferTarget, attachment, textureTarget, texture, level);
  DEBUG_GL_ERRORS;
}
...
inline void GL_GenRenderBuffers(int count, GL_Renderbuffer* rBufferArray)
{
...
/* Returns information about an active uniform variable for the specified
   program object. */
inline void GL_GetActiveUniform(GL_Program program, unsigned int index, int& size, GLenum& type, std::string& name)
{
  char buf[256];
  glGetActiveUniform(static_cast<unsigned int>(program), index, sizeof(buf) - 1, nullptr, &size, &type, buf);
  name = buf;
  DEBUG_GL_ERRORS;
}
...
inline GL_Program GL_GetCurrentProgram()
{
  ...
  return static_cast<GL_Program>((unsigned int)program);
}
...
{
  return static_cast<GL_FramebufferStatus::Enum>(glCheckFramebufferStatus((unsigned int)fbo));
}
...
{
  int i;
  glGetProgramiv(static_cast<unsigned int>(program), property, &i);
  DEBUG_GL_ERRORS;
  return i;
}
...
{
  int i;
  glGetShaderiv(static_cast<unsigned int>(shader), property, &i);
  DEBUG_GL_ERRORS;
  return i;
}
...
inline void GL_GetProgramInfoLog(GL_Program program, std::string& log)
{
  ...
  glGetProgramInfoLog(static_cast<unsigned int>(program), logSize, nullptr, logBuffer);
  log = logBuffer;
  delete[] logBuffer;
  DEBUG_GL_ERRORS;
}
...
inline void GL_GetShaderInfoLog(GL_Shader shader, std::string& log)
{
  ...
  auto logBuffer = new char[logSize];
  glGetShaderInfoLog(static_cast<unsigned int>(shader), logSize, nullptr, logBuffer);
  log = logBuffer;
  delete[] logBuffer;
  DEBUG_GL_ERRORS;
}
 ...
 * object. */
inline void GL_GetTexImage(GL_TextureTarget::Enum target, int level, GL_PixelFormat::Enum pixelFormat, GL_DataFormat::Enum dataFormat,
                           void* buffer)
{
...
inline int GL_GetTexLevelParameterI(GL_TextureTarget::Enum target, int level, GL_TextureParameterRead::Enum parameter)
{
  int i;
...
{
  return GL_GetTexLevelParameterI(GL_TextureTarget::T2D, 0, GL_TextureParameterRead::Height);
}
...
{
  return GL_GetTexLevelParameterI(GL_TextureTarget::T2D, 0, GL_TextureParameterRead::Width);
}

/* Return the location of a uniform variable. */
inline int GL_GetUniformLocation(GL_Program program, const char* name)
{
  int location = glGetUniformLocation(static_cast<unsigned int>(program), name);
  DEBUG_GL_ERRORS;
  return location;
}
...
inline void GL_NormalPointer(GL_VertexFormat::Enum format, int sizeofVertex, const void* normalData)
{
...
inline void GL_Ortho(float left, float right, float bottom, float top, float nearPlane, float farPlane)
{
...
inline void GL_ReadPixels(int x, int y, int width, int height, GL_PixelFormat::Enum pixelFormat, GL_DataFormat::Enum dataFormat, void* data)
{
...
inline void GL_ShaderSource(GL_Shader shader, const std::string& source)
{
  const int size = static_cast<int>(source.size());
  const char* src = source.c_str();
  glShaderSource(static_cast<unsigned int>(shader), 1, &src, &size);
  DEBUG_GL_ERRORS;
}

/* Set the current texture coordinates. */
inline void GL_TexCoord(float u) { glTexCoord1f(u); }

/* Set the current texture coordinates. */
inline void GL_TexCoord(float u, float v) { glTexCoord2f(u, v); }

/* Set the current texture coordinates. */
inline void GL_TexCoord(float u, float v, float w) { glTexCoord3f(u, v, w); }

/* Define an array of texture coordinates. */
inline void GL_TexCoordPointer(int componentsPerCoord, GL_VertexFormat::Enum format, size_t sizeofVertex, const void* data)
{
  glTexCoordPointer(componentsPerCoord, format, static_cast<int>(sizeofVertex), data);
  DEBUG_GL_ERRORS;
}

/* Specify a two-dimensional texture image. */
inline void GL_TexImage2D(GL_TextureTarget::Enum target, int level, GL_TextureFormat::Enum internalFormat, size_t width, size_t height,
                          GL_PixelFormat::Enum pixelFormat, GL_DataFormat::Enum dataFormat, const void* uploadData)
{
  glTexImage2D(target, level, internalFormat, static_cast<int>(width), static_cast<int>(height), 0, pixelFormat, dataFormat, uploadData);
  DEBUG_GL_ERRORS;
}

/* Specify a three-dimensional texture image. */
inline void GL_TexImage3D(GL_TextureTarget::Enum target, int level, GL_TextureFormat::Enum internalFormat, size_t width, size_t height,
                          size_t depth, GL_PixelFormat::Enum pixelFormat, GL_DataFormat::Enum dataFormat, const void* uploadData)
{
  glTexImage3D(target, level, internalFormat, static_cast<int>(width), static_cast<int>(height), static_cast<int>(depth), 0, pixelFormat,
...
inline void GL_TexWrapMode(GL_TextureTarget::Enum target, GL_TextureCoordinate::Enum coordinate, GL_TextureWrapMode::Enum mode)
{
  GLenum pName;
...
inline void GL_TexSubImage2D(GL_TextureTarget::Enum target, int level, int xOffset, int yOffset, size_t width, size_t height,
                             GL_PixelFormat::Enum pixelFormat, GL_DataFormat::Enum dataFormat, const void* uploadData)
{
  glTexSubImage2D(target, level, xOffset, yOffset, static_cast<int>(width), static_cast<int>(height), pixelFormat, dataFormat, uploadData);
  DEBUG_GL_ERRORS;
}

/* Specify a box-shaped subregion of a three-dimensional texture image. */
inline void GL_TexSubImage3D(GL_TextureTarget::Enum target, int level, int xOffset, int yOffset, int zOffset, size_t width, size_t height,
                             size_t depth, GL_PixelFormat::Enum pixelFormat, GL_DataFormat::Enum dataFormat, const void* uploadData)
{
  glTexSubImage3D(target, level, xOffset, yOffset, zOffset, static_cast<int>(width), static_cast<int>(height), static_cast<int>(depth),
...
inline void GL_TexMagFilter(GL_TextureTarget::Enum target, GL_TextureFilter::Enum filter)
{
...
inline void GL_TexMaxAnisotropy(GL_TextureTarget::Enum target, float anisotropy)
{
...
inline void GL_TexMinFilter(GL_TextureTarget::Enum target, GL_TextureFilterMip::Enum filter)
{
...
inline void GL_TexParameter(GL_TextureTarget::Enum target, GL_TextureParameter::Enum param, int paramValue)
{
...
/* Set texture parameters. */
inline void GL_TexParameter(GL_TextureTarget::Enum target, GL_TextureParameter::Enum param, float paramValue)
{
...
*/
inline void GL_UniformArray1(int location, size_t arraySize, const float* fv)
{
  glUniform1fv(location, static_cast<int>(arraySize), fv);
  DEBUG_GL_ERRORS;
}
...
/* Specify the value of a uniform variable for the current program.
   NONSTANDARD : Required name change in order to avoid overload conflict.
*/
inline void GL_UniformArray2(int location, size_t arraySize, const float* fv)
{
  glUniform2fv(location, static_cast<int>(arraySize), fv);
  DEBUG_GL_ERRORS;
}
...
/* Specify the value of a uniform variable for the current program.
   NONSTANDARD : Required name change in order to avoid overload conflict.
*/
inline void GL_UniformArray3(int location, size_t arraySize, const float* fv)
{
  glUniform3fv(location, static_cast<int>(arraySize), fv);
  DEBUG_GL_ERRORS;
}
...
/* Specify the value of a uniform variable for the current program.
   NONSTANDARD : Required name change in order to avoid overload conflict.
*/
inline void GL_UniformArray4(int location, size_t arraySize, const float* fv)
{
  glUniform4fv(location, static_cast<int>(arraySize), fv);
  DEBUG_GL_ERRORS;
}

inline void GL_UniformMatrix2(int location, const float* m)
{
  glUniformMatrix2fv(location, 1, false, m);
  DEBUG_GL_ERRORS;
}

inline void GL_UniformMatrix3(int location, const float* m)
{
  glUniformMatrix3fv(location, 1, false, m);
  DEBUG_GL_ERRORS;
}

inline void GL_UniformMatrix4(int location, const float* m)
{
...
{
  glUseProgram(static_cast<unsigned int>(program));
  DEBUG_GL_ERRORS;
}

inline void GL_ValidateProgram(GL_Program program)
{
  glValidateProgram(static_cast<unsigned int>(program));
  DEBUG_GL_ERRORS;
}

/* Specify a vertex of float components. */
inline void GL_Vertex(float x, float y) { glVertex2f(x, y); }

/* Specify a vertex of float components. */
inline void GL_Vertex(float x, float y, float z) { glVertex3f(x, y, z); }
...
inline void GL_VertexAttribPointer(unsigned int index, int valuesPerVertex, GL_DataFormat::Enum dataType, bool normalized,
                                   size_t sizeOfVertex, const void* data)
{
  glVertexAttribPointer(index, valuesPerVertex, dataType, normalized, static_cast<int>(sizeOfVertex), data);
  DEBUG_GL_ERRORS;
}

/* Define an array of vertex data. */
inline void GL_VertexPointer(int componentsPerVertex, GL_VertexFormat::Enum vf, size_t sizeOfVertex, const void* data)
{
  glVertexPointer(componentsPerVertex, vf, static_cast<int>(sizeOfVertex), data);
  DEBUG_GL_ERRORS;
}
...
namespace GLU
{
  ...
  inline GL_Texture CreateTexture2D(unsigned int width, unsigned int height, GL_TextureFormat::Enum internalFormat)
  {
    GL_Texture texture;
  ...
  inline void DrawQuad(float minX, float minY, float maxX, float maxY, float z = 0.0f)
  {
  ...
  /* Loads the given matrices into the ModelView and Projection stacks,
     respectively. */
  inline void SetMatrices(const float* modelView, const float* projection)
  {
    GL_MatrixMode(GL_Matrix::ModelView);
