...
namespace LTE {
  ...
  void Renderer_Clear(V4 const& clearColor = V4(0));

  /* Clears only the depth buffer. */
  void Renderer_ClearDepth(float depth = 1.0f);
  ...
  void Renderer_SetColor(Color const& color, float alpha = 1);
  ...
  void Renderer_PushDepthBuffer(GL_Texture texture);
