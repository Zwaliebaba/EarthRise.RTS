...
constexpr size_t kUpdatePeriod = 2;
...
AutoClassDerived(BeamImpl, Beam, Object, hitObject, float, length, float, age, float, targetLength, int, tick, bool, soundPlayed)
  ...
  struct RenderComponent : RenderableT
  {
    ...
    void Render(DrawState* state) const override
    {
      ...
      (*gShader)("t", self->age)("axis", Normalize(self->direction))("size", V2(6.f * self->width, self->length))("thinness", 10.f)(
  ...
  BeamImpl()
    : length(0),
      ...
      soundPlayed(false) { Drawable.renderable = static_cast<Renderable>(new RenderComponent(this)); }
  ...
  void OnUpdate(UpdateState& state) override
  {
    ...
    if (--tick <= 0)
    {
      ...
      targetLength = 0;
      float t;
      hitObject = GetContainer()->QueryInterior(ray, t, Damager.type->range, nullptr, true, RaycastCanCollideBidirectional, this);
    ...
    if (hitObject)
      Damager.Hit(this, hitObject, origin + direction * targetLength, state.dt);
...
Beam* Beam_Create() { return new BeamImpl; }
