...
using MissileBaseT = ObjectWrapper<Component_Damager<Component_Drawable<Component_Motion<Component_Orientation<ObjectWrapperTail<
  ObjectType_Missile>>>>>>;

AutoClassDerived(Missile, MissileBaseT, LightRef, light, Object, target, V3, targetOffset, Object, trail, V3, thrust, float, age)
  Sound sound;
  ...
  Missile(const Object_Missile_ArgRefs& args)
    : target(args.target),
      targetOffset(args.targetOffset),
      thrust(args.thrust),
      age(0) { Motion.velocity = args.velocity; }
  ...
  void OnUpdate(UpdateState& state) override
  {
    ...
    if (target)
    {
      ...
      Position impactPos;
      float impactTime = ComputeImpact(GetPos(), targetPos, V3(0), target->GetVelocity(), Damager.type->speed, &impactPos);

      if (impactTime < 0)
        impactPos = targetPos;

      Motion.velocity = Damager.type->speed * Mix(Normalize(Motion.velocity), Normalize(static_cast<V3>(impactPos - GetPos())),
                                                  1.0 - Exp(-0.5f * state.dt));
    }
    ...
    if (age > Damager.type->range / Damager.type->speed) { Delete(); }
  }
};

DERIVED_IMPLEMENT(Missile)

DefineFunction(Object_Missile) { return new Missile(args); }
