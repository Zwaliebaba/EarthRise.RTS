...
constexpr float kTrailLengthMult = 8;
constexpr float kAcceleration = 1;
const auto kBoostColor = V3(0.2f, 0.7f, 1.0f);
...
namespace
{
  ...
  void OnLoad()
  {
    ...
    (*gShaderInstance)(RenderStateSwitch_BlendModeAdditive)(RenderStateSwitch_ZWritableOff);
...
using ThrusterBaseT = ObjectWrapper<Component_Attachable<Component_BoundingBox<Component_Collidable<Component_Cullable<Component_Drawable<
  Component_Explodable<Component_Integrity<Component_Orientation<Component_Pluggable<Component_Scriptable<Component_Supertyped<
    ObjectWrapperTail<ObjectType_Thruster>>>>>>>>>>>>>;

AutoClassDerived(Thruster, ThrusterBaseT, float, activation, float, age, Color, color, float, cruise, float, thrust, LightRef, light)
  ...
  struct RenderComponent : RenderableT
  {
    ...
    void Render(DrawState* state) const override
    {
      ...
      Transform transform = self->GetTransform() * Transform_Translation(self->Supertyped.type->GetOffset());

      (*gShader)("axis", transform.look)("size", Length(transform.scale) * V2(0.5f, opacity * kTrailLengthMult))("t", self->age)(
  ...
  float GetBasePower() const { return Supertyped.type->GetPowerDrain(); }

  float GetPowerFraction() const override { return Pluggable.powerIn / GetBasePower(); }

  float GetOutput() { return GetPowerFraction() * activation; }

  float GetOvercharge() const { return Saturate(2.0f * (GetPowerFraction() - 1.0f)); }
  ...
  void OnMessage(Data& m) override
  {
    ...
    {
      const MessageThrustAngular& v = m.Convert<MessageThrustAngular>();
  ...
  void OnUpdate(UpdateState& state) override
  {
    ...
    if (!light)
    {
      ...
      light->Attachable.transform = Transform_Translation(Supertyped.type->GetOffset());
    }
    ...
    color = activation * Mix(Supertyped.type->GetColor(), kBoostColor, GetOvercharge());
    light->color = color;
  }

  void Thrust(const V3& dir) { thrust = Saturate(2.0f * Saturate(-Dot(GetLook(), dir)) - 0.5f); }

  void ThrustAngular(const V3& dir, float amount)
  {
    float torque = GetMaxTorque();
    parent->GetMotion()->torque += Normalize(dir) * torque * Saturate(amount);
  }
};

DERIVED_IMPLEMENT(Thruster)

Object Object_Thruster(const Item& type, ObjectT* parent)
{
  Reference<Thruster> self = new Thruster;
  self->SetSupertype(type);
  self->Drawable.renderable = static_cast<Renderable>(new Thruster::RenderComponent(self));
  ScriptFunction_Load("Object/Thruster:Init")->VoidCall(nullptr, DataRef(static_cast<Object>(self)));
  return self;
}
