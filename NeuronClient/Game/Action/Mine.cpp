...
namespace
{
  AutoClassDerived(ActionMine, ActionT, Object, object, Object, target, Position, point)
    ...
    void Execute(UpdateState& state) const override
    {
      ...
      Quantity quantity = Min(mine->quantity, static_cast<Quantity>(rate));
      root->AddItem(mine->item, quantity);
      mine->quantity -= quantity;

      ObjectT* container = object->GetContainer();
      if (container && quantity > 0) { Object_AddHistory(container, Event_Mined(object, target, mine->item, quantity)); }
    }

    String GetName() const override { return "Mine"; }
  };
}

DefineFunction(Action_Mine) { return new ActionMine(args.object, args.target, args.point); }
